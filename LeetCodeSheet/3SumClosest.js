...
var threeSumClosest = function (nums, target) {
    ...
    for (k = 0; k < len; k++) {
        ...
        j = len - 1;
        while (i < j) {
            if (Math.abs(target - (nums[i] + nums[k] + nums[j])) < diff) {
                diff = Math.abs(target - (nums[i] + nums[k] + nums[j]));
                mySum = nums[i] + nums[k] + nums[j];
                // console.log("calling ->", nums[i], nums[j], nums[k], mySum);
            }
            if (nums[i] + nums[k] + nums[j] - target > 0) {
                j--;
            } else if (nums[i] + nums[k] + nums[j] - target < 0) {
                i++;
            } else {
                break;
            }
